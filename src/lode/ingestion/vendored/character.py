"""Character-based text splitters, vendored from ``langchain-text-splitters``.

Copyright (c) LangChain, Inc. Licensed under the MIT License.

Source: ``langchain_text_splitters.character`` in the langchain repository

Pinned to upstream commit ``a2e53fda733c3ccc180b85fbbdb48893d770549f``
(2026-07-05, "feat(text-splitters): replace `mypy` by `ty` for type checking
(#38658)"):
<https://github.com/langchain-ai/langchain/blob/a2e53fda733c3ccc180b85fbbdb48893d770549f/libs/text-splitters/langchain_text_splitters/character.py>.
...
class CharacterTextSplitter(TextSplitter):
    ...
    def split_text(self, text: str) -> list[str]:
        ...
        lookaround_prefixes = ("(?=", "(?<!", "(?<=", "(?!")
        is_lookaround = self._is_separator_regex and any(self._separator.startswith(p) for p in lookaround_prefixes)
...
class RecursiveCharacterTextSplitter(TextSplitter):
    ...
    def get_separators_for_language(language: Language) -> list[str]:
        ...
        if language in Language._value2member_map_:  # pyright: ignore[attr-defined]
